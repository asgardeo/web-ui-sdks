 ...
 */
const SignInGeneric: FC<SignInProps> = (props: SignInProps): ReactElement => {
  const {
    basicAuthChildren,
    brandingProps,
    emailOtpChildren,
  ...
  const [brandingPreference, setComponentBranding] = useState<Branding>();

  console.log('auth Response Display');
  ...
  let copyrightText: string = keys.common.copyright;
  ...
  return (
    // <ThemeProvider theme={generateThemeSignIn(brandingPreference?.preference.theme)}>
    <UISignIn className="Box-asgardeoSignIn">
      {showLogo && !authContext?.isComponentLoading && (
      ...
      <>
        ...
        {/* <UISignIn.Footer
          ...
          items={[
            {
              children: (
                <UISignIn.Link href={brandingPreference.preference.urls.privacyPolicyURL}>
                  {keys.common.privacy.policy}
                </UISignIn.Link>
              ),
            },
            {
              children: (
                <UISignIn.Link href={brandingPreference.preference.urls.termsOfUseURL}>
                  {keys.common.terms.of.service}
                </UISignIn.Link>
              ),
            },
            {children: <UISignIn.Typography>{brandingPreference?.locale ?? DEFAULT_LOCALE}</UISignIn.Typography>},
          ]}
        /> */}
        {/* )} */}
      </>
